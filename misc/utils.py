    ...
    if t_now - t_last > min_delay:
      ...
      if n > 0:
        ...
        if displaytype == 's1k': # minutes/seconds for 1000 iters
          ...
          eprint("(TE/ET1k: %d:%02d / %d:%02d)" % tuple(outlist), end=" ")
        else:# displaytype == 'eta':
          t_done = t_now - t_start
          t_total = t_done / n * total
          outlist = list(divmod(t_done, 60)) + list(divmod(t_total - t_done, 60))
          eprint("(TE/ETA: %d:%02d / %d:%02d)" % tuple(outlist), end=" ")

      sys.stdout.flush()
      t_last = t_now
    yield item
  t_total = time.time() - t_start
  eprint("\r%s%d/%d (100.00%%) (took %d:%02d)" % ((desc, total, total) +
                                                   divmod(t_total, 60)))
