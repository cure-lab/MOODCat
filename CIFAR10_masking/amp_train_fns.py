...
#** 2021-11-21 adding LPIPS loss
from lpips_pytorch.lpips_pytorch import LPIPS, lpips
...
def GAN_training_function(G, D, GD, encoder, z_, y_, ema, state_dict, config, writer):
    def train(x, y, epoch, batch_size, target_map = None, r_mixup = 0.0, writer=writer, step_count=None):
                ...
                for accumulation_index in range(n_d_accu):#1
                    ...
                    if use_mixup_in_this_round:
                        ...
                        if (not config["full_batch_mixup"]) or (config["full_batch_mixup"] and (config["consistency_loss_and_augmentation"] or config["consistency_loss"]) ): # * go here
                            # split_D = False
                            D_fake, D_real , D_mixed, G_z, mixed,  D_middle_fake, D_middle_real, D_middle_mixed, target_map = GD(z_[:batch_size], y[counter],
                        ...
                        # y_nail = torch.ones_like(y[counter]) 
                        # y_macaw = torch.ones_like(y[counter])*2       
                        # G_z_nail = G(z_,y_nail.cuda())
                        # G_z_macaw = G(z_, y_macaw.cuda())
                        # print("plotting images")
                        # writer.add_image("real", torchvision.utils.make_grid(x.permute(1, 2, 0),nrow=8, padding=2,normalize=True),global_step=step_count)
                        # writer.add_image("fake", torchvision.utils.make_grid(G_z.permute(1, 2, 0), nrow=8, padding=2, normalize=True), global_step=step_count)
                        # writer.add_image("fake_nail", torchvision.utils.make_grid(G_z_nail.permute(1, 2, 0), nrow=8, padding=2, normalize=True), global_step=step_count)
                        # writer.add_image("fake_macaw", torchvision.utils.make_grid(G_z_macaw.permute(1, 2, 0), nrow=8, padding=2, normalize=True), global_step=step_count)
                        # writer.add_image("mixed", torchvision.utils.make_grid(mixed.permute(1, 2, 0), nrow=8, padding=2, normalize=True), global_step=step_count)
                        # writer.add_image("target_map", torchvision.utils.make_grid(target_map.permute(1, 2, 0), nrow=8, padding=2), global_step=step_count)
        ...
        for accumulation_index in range(config['num_G_accumulations']):
            ...
            G_fake, G_fake_middle, G_z = GD(z__[counter], y__[counter],return_G_z=True, train_G=True, split_D=config['split_D'], reference_x = x[counter] )
            ...
            with torch.cuda.amp.autocast():
                ...
                G_loss_lpips =  criterion_lpips(G_z, x[counter])

                G_loss = 0.5*G_loss_fake_middle + 0.5*G_loss_fake_2d + G_loss_lat + 5*G_loss_ssim + G_loss_lat + 3*G_loss_l2  + 2*G_loss_l1+G_loss_lpips
                ...
                # G_loss_lpips_item = G_loss_lpips.detach().item()
                # print("lpips:",G_loss_lpips_item)
        ...
        scaler.step(G.optim)
        scaler.step(encoder.module.optim)
        scaler.update()
        del G_loss
...
def save_and_sample(G, D,encoder, G_ema, z_, y_, fixed_z, fixed_y,
