...
from noridataset import ImageNetNoriDataset
...
def run(config):
    ...
    import neg_train_fns as train_fns
    ...
    config['resolution'] = 256#utils.imsize_dict[config['dataset']]
    print("RESOLUTION: ",config['resolution'])
    config['n_classes'] = 50#utils.nclass_dict[config['dataset']]
    config['G_activation'] = utils.activation_dict[config['G_nl']]
    config['D_activation'] = utils.activation_dict[config['D_nl']]
    config['mode_collapse_conditional'] = True
    ...
    for k in keys:
        print(k, ": ", config[k])
    # ** adding mode collapse images
    G = model.Generator(**config).to(device)

    D = model.Unet_Discriminator(**config).to(device)
    encoder = model.Encoder(isize=256, nz=128, nc=3, ndf=32).to(device)
    ...
    *[sum([p.data.nelement() for p in net.parameters()]) for net in [G,D,encoder]]))
    ...
    # ** this dozen of z_, y_ is unnecessary
    ...
    if config["dataset"]=="ImageNet":# imageNet100
        ...
        #dataset = ILSVRC_HDF5(root=root, transform=transform, load_in_mem=True)#hdf5 has already done the tranforms
        dataset = ImageNetNoriDataset(
            "/data/yangyijun/datasets/imagenet/imagenet50.train.nori.list",
            transform,
        )
        data_loader = DataLoader(dataset,batch_size*config["num_D_accumulations"],drop_last=True,num_workers=32, pin_memory=True, shuffle=True)
        loaders = [data_loader]
    ...
    for epoch in range(state_dict['epoch'], config['num_epochs']):
        ...
        for i, batch_data in enumerate(pbar):# * the i indicates one batch data
            ...
            metrics, z_ = train(x, y, state_dict["epoch"], batch_size , target_map = target_map, r_mixup = r_mixup, writer=writer, step_count=i) # * mixup_rate=0.5
          ...
          # Save weights and copies as configured at specified interval
            if not (state_dict['itr'] % config['save_every']):
                ...
                if config['G_eval_mode']:
                    ...
                    if config['ema']:
                        G_ema.eval()
                    train_fns.save_and_sample(G, D,encoder, G_ema, x[0:64], y[0:64], z_[0:64], y[0:64],
