...
import osr_random1_utils as utils
...
def run(config):
    ...
    # exit()
    config['n_classes'] = 6#utils.nclass_dict[config['dataset']]
...
# TODO: modify the data augmentation part
    if config["dataset"]=="CIFAR10":# imageNet100
        ...
        #* Here we randomly select 6 classes from CIFAR10 as known classes, the other 4 as unknown to meet OSR experimential setting the same as C2AE
        #* Random selected combination1:[0, 7, 2, 8, 1, 9] [6, 4, 5, 3]
        #* Random selected combination2:[1, 2, 5, 6, 7, 4] [0, 3, 9, 8]
        #* Random selected combination3:[4, 8, 7, 0, 6, 2] [1, 3, 5, 9]
        comb = 'random1'
        root = os.path.join(config["data_folder"],'split_cifar10',comb,'train')
        train_dataset = torchvision.datasets.ImageFolder(root=root, transform=data_transform)
        # test_dataset = torchvision.datasets.CIFAR10(root=root, transform=data_transform, download=True, train=False)
        root = os.path.join(config["data_folder"],'split_cifar10',comb,'test')
        test_dataset = torchvision.datasets.ImageFolder(root=root, transform=data_transform)
        ...
        test_dataset_loader = torch.utils.data.DataLoader(
            test_dataset, batch_size*config["num_D_accumulations"],drop_last=True,num_workers=32, pin_memory=True, shuffle=False)
