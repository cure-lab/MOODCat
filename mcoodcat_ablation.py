...
import misc.mask_methods as mask_methods
...
def tpr_fpr_process(a_name, min_dists, this_score, this_y, all_combinations,
    ...
    if _MULTI_PROCESS_INNER:
       ...
       fpr, tpr, prec, recall, iprec, irecall = tpr_fpr_single_attack(
           min_dists, this_score, this_y, all_combinations, d_names) 
...
def save_img(data, name, ncol):
    dirname = os.path.dirname(name)
    if not os.path.exists(dirname):
        os.makedirs(dirname)
    grid = torchvision.utils.make_grid(data, ncol, normalize=True)
...
def run(config):
    ...
    parser.add_argument("--test_detector", type=int, nargs="+", default=[5, 9])
    parser.add_argument("--mask_method", type=str)
    args = parser.parse_known_args()[0]
    print(args.test_detector)
    assert args.mask_method in [
        "random", "high_fix", "low_fix", "board", "shuffle", "zero_mask",
        "board0.3"
    ]
    print(f">>>>>>>>>>>>> This is for mask: {args.mask_method} <<<<<<<<<<<<<<")
    ...
    elif args.mask_method == "high_fix":
        erasing_transforms = mask_methods.FixCenterMask(7, 25, 7, 25)
    elif args.mask_method == "low_fix":
        erasing_transforms = mask_methods.FixCenterMask(11, 21, 11, 21)
    elif args.mask_method == "board":
        erasing_transforms = mask_methods.MosaicMask(0.5, 8, 8, 4)
    elif args.mask_method == "board0.3":
        erasing_transforms = mask_methods.MosaicMask(0.7, 8, 8, 4)
    elif args.mask_method == "shuffle":
        erasing_transforms = mask_methods.RandomBoardShuffle(8, 8, 4)
    elif args.mask_method == "zero_mask":
        erasing_transforms = lambda x: x
    else:
        raise NotImplementedError()
    ...
    for data_id, batch_data in enumerate(pbar):# * the i indicates one batch data
        ...
        if data_id == 0:
            x_save = torch.cat([x[:16], input_x[:16], synthesis[:16]], dim=0)
            save_img(x_save, f"results/mask_example/{BENCHMARK}_{OOD}_{args.mask_method}_in_d.png", 8)
    ...
    for data_id, batch_data in enumerate(pbar):# * the i indicates one batch data
        ...
        if data_id == 0:
            x_save = torch.cat([x_out[:16], input_x[:16], synthesis[:16]], dim=0)
            save_img(x_save, f"./mask_example/{BENCHMARK}_{OOD}_{args.mask_method}_out_d.png", 8)
...
def eval_auc(positive_tank_1, positive_tank_2, positive_tank_3, positive_tank_4,
    ...
    if False:
        x_train_1 = np.around((x_train_1-x_train_1.mean())/x_train_1.var(), decimals=0)
        print(x_train_1.max(), x_train_1.min())

        x_train_2 = np.around((x_train_2-x_train_2.mean())/x_train_2.var(), decimals=0)
        print(x_train_2.max(), x_train_2.min())

        x_train_3 = np.around((x_train_3-x_train_3.mean())/x_train_3.var(), decimals=0)
        ...
        x_train_5 = np.around((x_train_5-x_train_5.mean())/x_train_5.var(), decimals=1)
        ...
        # x_train_8 = np.around((x_train_8-x_train_8.mean())/x_train_8.var(), decimals=1)
    ...
    # encoder_detector = np.load("./patch8_s4/KID_patch8_s4_ood_svhn.npy", allow_pickle=True)
    # import ipdb; ipdb.set_trace()

    # detector_dict['1']= torch.from_numpy(x_train_1)
    # detector_dict['2'] = torch.from_numpy(x_train_2)
    # detector_dict['3'] = -torch.from_numpy(x_train_3)
    # detector_dict['4'] = torch.from_numpy(x_train_4)
    # detector_dict['5'] = torch.from_numpy(x_train_5)
    # detector_dict['6'] = torch.from_numpy(x_train_6)
    # detector_dict['7'] = torch.from_numpy(x_train_7)
    # detector_dict['8'] = torch.from_numpy(x_train_8)

    for di in DETECTOR_CHOICE:
    ...
    fpr_7, tpr_7, thresholds_7 = roc_curve(y_train, x_train_7)
    print("auc of detector 7:",roc_auc_score(y_train, x_train_7))
    print("length of thresh 7:",len(thresholds_7))

    fpr_8, tpr_8, thresholds_8 = roc_curve(y_train, x_train_8)
    print("auc of detector 8:",roc_auc_score(y_train, x_train_8))
    print("length of thresh 8:",len(thresholds_8))

    fpr_9, tpr_9, thresholds_9 = roc_curve(y_train, x_train_9)
    ...
    dirname = f"results/roc_curve_scood_mask/{args.ind_dataset}_{tag_str}"
    if not os.path.exists(dirname):
        os.makedirs(dirname)
    save_name = os.path.join(
        dirname,
        f"cascade_piq_detector_roc_curve_{OOD}_{tag_str}_{args.mask_method}.pdf")
    # import ipdb; ipdb.set_trace()
