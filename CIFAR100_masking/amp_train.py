...
def run(config):
    ...
    import amp_train_fns as train_fns
    ...
    config['n_classes'] = 100#utils.nclass_dict[config['dataset']]
    ...
    encoder = model.Encoder(isize=128, nz=128, nc=3, ndf=64).to(device)
    ...
    *[sum([p.data.nelement() for p in net.parameters()]) for net in [G,D,encoder]]))
    ...
    if config["dataset"]=="FFHQ":
        ...
        data_loader = DataLoader(dataset,batch_size*config["num_D_accumulations"],drop_last=True,num_workers=32)#,shuffle=False)
        loaders = [data_loader]
    elif config["dataset"]=="ImageNet":# imageNet100
        batch_size = config['batch_size']
        # transform=transforms.Compose(
        #         [ CenterCropLongEdge(),
        #             transforms.Resize(config["resolution"]),
    ...
    if config['which_train_fn'] == 'GAN': # which_train_fn ==GAN
        train = train_fns.GAN_training_function(G, D, GD, encoder, z_, y_,
