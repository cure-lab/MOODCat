...
def run(config):
	...
	config['n_classes'] = 43#utils.nclass_dict[config['dataset']]
	...
	dataset = torchvision.datasets.SVHN(root="/data/yangyijun/projects/adaptive attacks/dataset", split='test', transform = data_transform, download=True)
	...
	for epoch in range(state_dict['epoch'], config['num_epochs']):
		...
		for i, batch_data in enumerate(pbar):# * the i indicates one batch data
			...
			torchvision.utils.save_image(reference_x[0:64], "./0124_masking_generator_results/"+checkpoint_name+"_svhn_real_"+str(i)+"_50per.png", nrow=8, normalize=True)
			del reference_x 
			synthesis = G(z, G.shared(y))
			torchvision.utils.save_image(synthesis[0:64], "./0124_masking_generator_results/"+checkpoint_name+"_svhn_synthesis_"+str(i)+"_50per.png", nrow=8, normalize=True)
			del synthesis
			ones_synthesis = G(z, G.shared(ones*2))
			ones_synthesis = ones_synthesis.data.cpu()
			torchvision.utils.save_image(ones_synthesis[0:64], "./0124_masking_generator_results/"+checkpoint_name+"_svhn_ones_synthesis_"+str(i)+"_50per.png", nrow=8, normalize=True)
			del ones_synthesis 
			zeros_synthesis = G(z, G.shared(zeros*4))
			zeros_synthesis = zeros_synthesis.data.cpu()
			torchvision.utils.save_image(zeros_synthesis[0:64], "./0124_masking_generator_results/"+checkpoint_name+"_svhn_zeros_synthesis_"+str(i)+"_50per.png", nrow=8, normalize=True)
			del zeros_synthesis
