...
def run(config):
    ...
    # exit()
    config['n_classes'] = 200#utils.nclass_dict[config['dataset']]
...
# TODO: modify the data augmentation part
    if config["dataset"]=="CIFAR100":# imageNet100
        ...
        train_dataset = torchvision.datasets.CIFAR100(root=root, transform=data_transform, download=True, train=True)
        test_dataset = torchvision.datasets.CIFAR100(root=root, transform=data_transform, download=True, train=False)
